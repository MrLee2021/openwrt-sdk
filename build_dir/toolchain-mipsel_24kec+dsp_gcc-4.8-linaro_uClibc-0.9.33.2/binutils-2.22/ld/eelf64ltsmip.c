...
static lang_output_section_statement_type *gldelf64ltsmip_place_orphan
...
static char *
gldelf64ltsmip_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
struct gldelf64ltsmip_ld_so_conf
{
  char *path;
  size_t len, alloc;
};
...
static bfd_boolean
gldelf64ltsmip_parse_ld_so_conf
     (struct gldelf64ltsmip_ld_so_conf *info, const char *filename);

static void
gldelf64ltsmip_parse_ld_so_conf_include
     (struct gldelf64ltsmip_ld_so_conf *info, const char *filename,
      const char *pattern)
{
  char *newp = NULL;
  ...
  if (glob (pattern, 0, NULL, &gl) == 0)
	...
	gldelf64ltsmip_parse_ld_so_conf (info, gl.gl_pathv[i]);
      globfree (&gl);
    }
#else
  /* If we do not have glob, treat the pattern as a literal filename.  */
  gldelf64ltsmip_parse_ld_so_conf (info, pattern);
#endif
      ...
      if (CONST_STRNEQ (p, "include") && (p[7] == ' ' || p[7] == '\t'))
	{
	  ...
	  do
		...
		++p;
	      ...
	      *p++ = '\0';
	      if (dir[0] != '\0')
		gldelf64ltsmip_parse_ld_so_conf_include (info, filename,
							       dir);
	    }
	  while (c != '\0');
	}
...
static bfd_boolean
gldelf64ltsmip_check_ld_so_conf (const char *name, int force)
{
  ...
  if (! initialized)
    {
      char *tmppath;
      struct gldelf64ltsmip_ld_so_conf info;
      ...
      tmppath = concat (ld_sysroot, "/home/ubuntu/openwrt-sdk/staging_dir/toolchain-mipsel_24kec+dsp_gcc-4.8-linaro_uClibc-0.9.33.2/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldelf64ltsmip_parse_ld_so_conf (&info, tmppath))
	{
	  free (tmppath);
	  tmppath = concat (ld_sysroot, "/etc/ld.so.conf",
			    (const char *) NULL);
	  gldelf64ltsmip_parse_ld_so_conf (&info, tmppath);
	}
      free (tmppath);

      if (info.path)
	{
	  char *d = gldelf64ltsmip_add_sysroot (info.path);
	  free (info.path);
	  ld_so_conf = d;
	}
      initialized = TRUE;
  ...
  needed.by = NULL;
  needed.name = name;
  return gldelf64ltsmip_search_needed (ld_so_conf, &needed, force);
}
...
gldelf64ltsmip_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf64ltsmip_write_build_id_section (bfd *abfd)
{
...
gldelf64ltsmip_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf64ltsmip_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf64ltsmip_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf64ltsmip_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  if (gldelf64ltsmip_search_needed (command_line.rpath,
						  &n, force))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      char *tmpname = gldelf64ltsmip_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf64ltsmip_search_needed (tmpname,
							      &n,
							      force));
	      free (tmpname);
	    }
	  ...
	  if (gldelf64ltsmip_check_ld_so_conf (l->name, force))
	    break;
...
static void 
gldelf64ltsmip_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf64ltsmip_append_to_separated_string (&depaudit, cp);
...
gldelf64ltsmip_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
  	gldelf64ltsmip_append_to_separated_string (&audit, optarg); 
	break;

    case 'P':
	gldelf64ltsmip_append_to_separated_string (&depaudit, optarg);
	break;
