...
static lang_output_section_statement_type *gldelf64btsmip_place_orphan
...
static char *
gldelf64btsmip_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
struct gldelf64btsmip_ld_so_conf
{
  char *path;
  size_t len, alloc;
};
...
static bfd_boolean
gldelf64btsmip_parse_ld_so_conf
     (struct gldelf64btsmip_ld_so_conf *info, const char *filename);

static void
gldelf64btsmip_parse_ld_so_conf_include
     (struct gldelf64btsmip_ld_so_conf *info, const char *filename,
      const char *pattern)
{
  char *newp = NULL;
  ...
  if (glob (pattern, 0, NULL, &gl) == 0)
	...
	gldelf64btsmip_parse_ld_so_conf (info, gl.gl_pathv[i]);
      globfree (&gl);
    }
#else
  /* If we do not have glob, treat the pattern as a literal filename.  */
  gldelf64btsmip_parse_ld_so_conf (info, pattern);
#endif
      ...
      if (CONST_STRNEQ (p, "include") && (p[7] == ' ' || p[7] == '\t'))
	{
	  ...
	  do
		...
		++p;
	      ...
	      *p++ = '\0';
	      if (dir[0] != '\0')
		gldelf64btsmip_parse_ld_so_conf_include (info, filename,
							       dir);
	    }
	  while (c != '\0');
	}
...
static bfd_boolean
gldelf64btsmip_check_ld_so_conf (const char *name, int force)
{
  ...
  if (! initialized)
    {
      char *tmppath;
      struct gldelf64btsmip_ld_so_conf info;
      ...
      tmppath = concat (ld_sysroot, "/home/ubuntu/openwrt-sdk/staging_dir/toolchain-mipsel_24kec+dsp_gcc-4.8-linaro_uClibc-0.9.33.2/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldelf64btsmip_parse_ld_so_conf (&info, tmppath))
	{
	  free (tmppath);
	  tmppath = concat (ld_sysroot, "/etc/ld.so.conf",
			    (const char *) NULL);
	  gldelf64btsmip_parse_ld_so_conf (&info, tmppath);
	}
      free (tmppath);

      if (info.path)
	{
	  char *d = gldelf64btsmip_add_sysroot (info.path);
	  free (info.path);
	  ld_so_conf = d;
	}
      initialized = TRUE;
  ...
  needed.by = NULL;
  needed.name = name;
  return gldelf64btsmip_search_needed (ld_so_conf, &needed, force);
}
...
gldelf64btsmip_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldelf64btsmip_write_build_id_section (bfd *abfd)
{
...
gldelf64btsmip_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldelf64btsmip_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldelf64btsmip_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf64btsmip_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  if (gldelf64btsmip_search_needed (command_line.rpath,
						  &n, force))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      char *tmpname = gldelf64btsmip_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf64btsmip_search_needed (tmpname,
							      &n,
							      force));
	      free (tmpname);
	    }
	  ...
	  if (gldelf64btsmip_check_ld_so_conf (l->name, force))
	    break;
...
static void 
gldelf64btsmip_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf64btsmip_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(mips)\n\
ENTRY(__start)\n\
SEARCH_DIR(\"/home/ubuntu/openwrt-sdk/staging_dir/toolchain-mipsel_24kec+dsp_gcc-4.8-linaro_uClibc-0.9.33.2/mipsel-openwrt-linux-uclibc/lib64\"); SEARCH_DIR(\"=/usr/local/lib64\"); SEARCH_DIR(\"=/lib64\"); SEARCH_DIR(\"=/usr/lib64\"); SEARCH_DIR(\"=/usr/local/lib\"); SEARCH_DIR(\"=/lib\"); SEARCH_DIR(\"=/usr/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf64btsmip_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
  	gldelf64btsmip_append_to_separated_string (&audit, optarg); 
	break;

    case 'P':
	gldelf64btsmip_append_to_separated_string (&depaudit, optarg);
	break;
